...
export default class Level5 extends Phaser.Scene {
  ...
  levelNumber: number = 5; // default, can be overridden
  // ---------------------------------------------------------------------------
  // 💰 Currency, Lives & Game State
  // ---------------------------------------------------------------------------
  vineBalance: number = 120; // Starting VINE
...
const vineText = this.add.text(0, 0, '$VINE: 120', textStyle).setScale(0.5);
...
const campaignBtn = this.createStyledButton(
  ...
  () => {
    ...
    if (this.walletAddress && this.vineBalance > 0) {
      ...
      window.dispatchEvent(new CustomEvent('upgrade-campaign', {
        detail: { level: 6 } // adjust level dynamically if needed
      }));
    }
