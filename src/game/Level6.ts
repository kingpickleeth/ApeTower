...
export default class Level6 extends Phaser.Scene {
  ...
  levelNumber: number = 6; // default, can be overridden
  // ---------------------------------------------------------------------------
  // 💰 Currency, Lives & Game State
  // ---------------------------------------------------------------------------
  vineBalance: number = 140; // Starting VINE
...
const vineText = this.add.text(0, 0, '$VINE: 140', textStyle).setScale(0.5);
...
const campaignBtn = this.createStyledButton(
  ...
  () => {
    ...
    if (this.walletAddress && this.vineBalance > 0) {
      ...
      window.dispatchEvent(new CustomEvent('upgrade-campaign', {
        detail: { level: 7 } // adjust level dynamically if needed
      }));
    }
