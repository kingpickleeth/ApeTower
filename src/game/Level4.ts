...
export default class Level4 extends Phaser.Scene {
  ...
  levelNumber: number = 4; // default, can be overridden
  // ---------------------------------------------------------------------------
  // 💰 Currency, Lives & Game State
  // ---------------------------------------------------------------------------
  vineBalance: number = 100; // Starting VINE
...
this.load.start();
  ...
  const pathTiles = [
    ...
    [3, 2], [3, 3], [4, 3], [5, 3], [6, 3],
    [6, 2], [7, 2], [8, 2],
    [8, 3], [8, 4], [7, 4], [7,5],[6,5], [5,5], [4,5],[3,5],[2,5], [1,5], [0,5]
  ];
...
const vineText = this.add.text(0, 0, '$VINE: 100', textStyle).setScale(0.5);
...
const campaignBtn = this.createStyledButton(
  ...
  () => {
    ...
    if (this.walletAddress && this.vineBalance > 0) {
      ...
      window.dispatchEvent(new CustomEvent('upgrade-campaign', {
        detail: { level: 5 } // adjust level dynamically if needed
      }));
    }
